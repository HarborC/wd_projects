...
def loadCameras(poses, viewpoint_stack, interp_fovs=None):
    ...
    # load interpolated poses
        ...
        for idx in range(poses.shape[0]):                                 
            ...
            if interp_fovs is not None:
                fovx, fovy = interp_fovs
                if idx < len(fovx):
                    viewpoint_stack[idx].FoVx = float(fovx[idx])
                    viewpoint_stack[idx].FoVy = float(fovy[idx])
                    viewpoint_stack[idx].projection_matrix = getProjectionMatrix(znear=viewpoint_stack[idx].znear, zfar=viewpoint_stack[idx].zfar, fovX=viewpoint_stack[idx].FoVx, fovY=viewpoint_stack[idx].FoVy).transpose(0,1).cuda()
...
def fetchPly(path):
    ...
    # Handle missing normals - use zeros if not present
...
def readColmapSceneInfo(path, images, eval, args, llffhold=8):
    ...
    # Try specific sparse folder first, then generic sparse
    sparse_folder = f"sparse_{args.n_views}"
    if not os.path.exists(os.path.join(path, sparse_folder)):
        if os.path.exists(os.path.join(path, "sparse")):
            sparse_folder = "sparse"
        else:
            # Fallback for compatibility or error later
            pass

    if eval:
        cameras_extrinsic_file = os.path.join(path, sparse_folder, "1", "images.txt")
        cameras_intrinsic_file = os.path.join(path, sparse_folder, "1", "cameras.txt")
    else:
        cameras_extrinsic_file = os.path.join(path, sparse_folder, "0", "images.txt")
        cameras_intrinsic_file = os.path.join(path, sparse_folder, "0", "cameras.txt")
